...
if(scalar(@ARGV) == 0){
    die "This calculates venn diagram for trio data given father, mother and child bed files. If mode is touch, it use 1bp overlap scheme, otherwise 50% reciprocal overlap\nUsage: $0 <f_bed> <m_bed> <c_bed> <mode>\n";
}

my ($f, $m, $c, $mode) = @ARGV;

my $com = "~/bin/intersectBed.reciprocal50.sh";
my $com_v = "~/bin/intersectBed.v.sh";

if($mode eq "touch"){
    $com = "~/bin/intersectBed.touch.sh";
    $com_v = "~/bin/intersectBed.v1.sh";
}
